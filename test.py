...
def test(data,
    ...
    if not training:
        ...
        dataloader,dataset = create_dataloader_rgb_ir(val_path_rgb, val_path_ir, imgsz, batch_size, gs, opt, pad=0.5, rect=True, prefix=colorstr(f'{task}: '))
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        img_rgb = img[:, :3, :, :]
        # img_ir = torch.zeros_like(img_rgb, device=img_rgb.device)
        img_ir = img[:, 3:, :, :]
        # img_rgb =torch.zeros_like(img_ir, device=img_ir.device)
        # if index_path < 451:
        #     # print(paths[index_path])
        #     for path in paths:
        #         # print(path)
        #         matching_filenames.append(path)
        #         index_path += 1
        # 10%的测试数据没有配对的图像，90%是配对的

        with torch.no_grad():
            # Run model
            t = time_synchronized()
            # print("=====augment:",augment)
            out= model(img_rgb, img_ir, augment=augment)  # inference and training outputs
            t0 += time_synchronized() - t
            # print("out.shape:",len(out[0]))
            train_out = out[0][2]
            out = out[0][0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/yzx/data/ICAFusion/runs/train/exp43/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--path-files',  type=str, default='/home/yzx/data/ICAFusion/runs/test/exp74/labels/validation_cvc.txt', help='test path file')
    ...
    p = "/home/yzx/data/datasets/KAIST/labels/test/"
